...
export class Collector {
  ...
  async execute(run: Run, signal: AbortSignal) {
    ...
    const normalized = normalize(raw, selection), analysis = analyze(normalized);
    const { collectedAt: _collectedAt, recentLatestAt: _recentLatestAt, ...availability } = normalized.dataWindow;
    const checksum = createHash('sha256').update(JSON.stringify({ songs: normalized.songs, availability, selection: selection ? { items: selection.items, source: selection.source } : null, algorithmVersion: analysis.algorithmVersion })).digest('hex');
    ...
    if (unchanged) snapshot = previous;
      ...
      snapshot = { ...analysis, snapshotId: randomUUID(), userId: run.userId, createdAt: new Date().toISOString(), checksum, insights: [], modules: buildModules(normalized, analysis, selection) };

...
后续输入2验证已完成：按`trackIds.at`降序选取50首候选集合，通过配置API保存、触发真实采集并生成双模块快照；50首均在当前红心列表，收藏时间仍全部未知。与长期Top100交集8首。JSON/CSV已生成并独立核对；这不等于at语义或最近一周窗口已验证。字段覆盖及下一阶段画像算法见[双输入实测导出与画像决策](data-export-and-profile.md)。

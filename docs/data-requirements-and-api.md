...
后续客户端核验已支持周/月汇总分钟单位、2025年度playDuration秒单位（整小时显示精度）及累计totalDuration秒单位（分钟显示精度）；音乐与播客分开可解释原汇总差异。详情见[时长核验](research/listening-duration-verification-2026-09-18.md)。累计统计范围和内容类型、单曲单位、覆盖率、时间契约及playNum去重语义仍需验证。collect含义、自订歌单at和完整历史仍未解决。旧导出保持不变。

...
## 探索前的候选评估（当前状态见文末实测更新）

以下表格记录探索前的评估。2026-09-18已通过可选只读模式验证多数来源，当前状态以文末实测更新和研究记录为准；未确认的语义不作为算法强依赖。
...
相关指导：[产品规范](product-spec.md)、[原完成指南](design/项目完成指南.md)、[字段补全与红心实测](metadata-enrichment.md)、[导出与画像决策](history/input-export-and-profile-2026-09-17.md)、[隐私约束](privacy.md)、[优化路线](roadmap.md)。


## 2026-09-18 实测更新（优先于上表旧候选状态）

本轮验证38个不同只读端点，详见[探索记录](research/account-api-exploration-2026-09-18.md)。上表是此前候选评估；本节更新当前状态，这些研究来源尚未接入常规算法。

- 首次收听4首中3首非空，两首与百科记忆一致；红心时间与歌单at及受控重新红心样本一致。
- 周/月/年报告、实时报告、各20首周期排行、6年聚合返回。响应窗口不同，部分时长汇总与明细不一致，单位尚未全确认。当日歌曲data为空。
- 关注艺人33位、订阅专辑11张；最近专辑和歌单各20项；3张自有歌单及各20首分页详情可读取。
- 艺人资料、20张专辑、50首热门歌曲、20位相似艺人返回；曲风28个顶层项、5项平台偏好及关联作品样本返回。
- 普通album返回26首；album/detail实际是数字专辑商品详情，本样本业务404，不能当普通专辑通用替代。歌曲全平台红心人数返回。

后续客户端核验已支持周/月汇总分钟单位、2025年度playDuration秒单位（整小时显示精度）及累计totalDuration秒单位（分钟显示精度）；单曲musicTotalPlayDto.duration分钟单位已获用户明确确认。音乐与播客分开可解释原汇总差异。详情见[时长核验](research/listening-duration-verification-2026-09-18.md)。累计统计范围和内容类型、取整规则、时间契约及playNum去重语义仍需验证。collect含义、自订歌单at和完整历史仍未解决。旧导出保持不变。

专用首听接口还提供最多播放日及当天次数、红心日期、常听时段；第三个歌曲样本的三项均已由账号持有人确认。它们是已实测、未接入常规输出的增强字段；其他歌曲可能为空，需登记覆盖率及来源，不从样本确认推断完整历史。

已完成双模块全部歌曲的[批量覆盖核验](research/song-memory-coverage-2026-09-18.md)：142首去重，140首非空、2首空响应、零失败；首听/累计次数/原duration覆盖140首，最多日和常听时段124首；非空已红心127首均有红心时间。确认的是本账号本次导出目标的接口覆盖，不是逐曲客户端准确性或全曲库覆盖。

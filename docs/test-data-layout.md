...
analyses/aesthetic-v2/<计算时间>/是一次派生审美计算：report.json完整结果、report.md可读画像、input-manifest.json来源和单位。latest.json仅索引最新计算，既不是快照UUID，也不是新采集时间；不改写exports或reviews。输出含私人信息，仅留本机。
